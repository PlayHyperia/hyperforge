...
// Explicit real-asset regression: the default manifest is intentionally not the
// review52 bank. This exercises actual worker output and retained triangles,
// without making the review asset overlay a production default.
const enabled = process.env.ASSETS_DIR?.includes(
  "/terrain-pond-review52-UNQUALIFIED/assets",
);

describe.skipIf(!enabled)("review52 retained pond grass work budget", () => {
  it("grounds the full native-failing northwest cell at near LOD without increasing its work cap", async () => {
    ...
    try {
      ...
      const nodes = [250, 350].map((z) =>
        tree.createNode(null, null, 100, 350, z, 4),
      );
      ...
      manager.setPlayerPosition(337.5, 287.5);
      manager.onNodeNeedsGeometry(nodes[0]);
      const key = "gcell_v1_13_11";
      ...
      if (result.status === "defer") {
        ...
        for (let i = 0; i < projected.count; i++) {
          ...
          maximumClumpWork = Math.max(maximumClumpWork, one.receipt.workUnits);
        }
        evidence(
          "REVIEW52_LEGACY_ORACLE_ATTRIBUTION",
          JSON.stringify({
      ...
      evidence(
        "REVIEW52_NORTHWEST_FULL_PIPELINE",
        JSON.stringify({
      ...
      expect(pipeline.state.result.sourceIndices).toEqual(result.sourceIndices);
