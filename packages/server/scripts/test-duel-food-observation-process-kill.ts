...
const CHILD_DATABASE_URL = "DUEL_FOOD_KILL_DATABASE_URL";
const CHILD_CONTEXT = "DUEL_FOOD_KILL_CONTEXT";
const PLAYER_ID = "duel-food-kill-agent";
const OPPONENT_ID = "duel-food-kill-opponent";

type ChildContext = {
  foodOperationId: string;
...
async function runAuthorityChild(): Promise<never> {
  ...
  const request = {
    operationId: context.foodOperationId,
    playerId: PLAYER_ID,
    itemId: "lobster",
    healAmount: 12,
  ...
  const staged =
    await databaseSystem.commitFoodConsumptionOperationAsync(request);
  assert(staged.status === "pending", "food custody did not stage");
  const completed =
    await databaseSystem.completeFoodConsumptionOperationAsync(request);
  assert(completed.status === "completed", "food effect did not complete");
  assert(completed.healedAmount === 12, "food effect healed the wrong amount");

  // This marker is emitted only after the serializable authority transaction
  // returned. The process is killed before any presentation ledger exists.
...
async function runParent(): Promise<void> {
  const baseDatabaseUrl =
    process.env.DUEL_FOOD_OBSERVATION_TEST_DATABASE_URL?.trim() ?? "";
  assert(
    baseDatabaseUrl,
    "DUEL_FOOD_OBSERVATION_TEST_DATABASE_URL is required",
  );

  const databaseName = `hyperia_duel_food_kill_${process.pid}_${Date.now().toString(36)}`;
  ...
  try {
    ...
    await db.insert(schema.users).values({
      id: "duel-food-kill-account",
      name: "Duel Food Kill Account",
      roles: "user",
      createdAt: "2026-08-25T00:00:00.000Z",
    });
    await db.insert(schema.characters).values({
      id: PLAYER_ID,
      accountId: "duel-food-kill-account",
      name: "Duel Food Kill Agent",
      isAgent: 1,
      health: 30,
      maxHealth: 60,
    ...
    const context: ChildContext = {
      foodOperationId: `food:${randomUUID()}`,
      observation: {
        operationId: randomUUID(),
        tick: 11,
        observedAt: Date.now(),
        cycleId: "cycle-food-process-kill",
        duelId: "duel-food-process-kill",
        actorId: PLAYER_ID,
    ...
    const authorityRows = await pool.query<{
      ...
      `SELECT
         (SELECT health FROM characters WHERE id = $1)::int AS health,
         (SELECT count(*) FROM inventory WHERE "playerId" = $1)::int AS "inventoryRows",
      ...
      [PLAYER_ID, context.foodOperationId, context.observation.operationId],
    );
    assert(authorityRows.rows[0]?.health === 42, "durable health is not 42");
    assert(
      authorityRows.rows[0]?.inventoryRows === 0,
      "food custody was not durably consumed",
    );
    assert(
    ...
    assert(recovered[0]?.action === "food", "recovered action is not food");
    assert(recovered[0]?.amount === 12, "recovered heal amount is not exact");
    ...
    console.log(
      JSON.stringify({
        status: "passed",
        boundary: "authority_commit_before_publication_process_kill",
        signal: childResult.signal,
        health: authorityRows.rows[0].health,
        inventoryRows: authorityRows.rows[0].inventoryRows,
        ...
        recoveredAmount: recovered[0]?.amount,
