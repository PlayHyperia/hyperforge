...
- [ ] Next test field-only fan-direction coherence without reducing plants, geometry counts or quality; reject repeated comb patterns or coverage loss. Any orientation change must re-run swept grounding, road clearance and actual retained masks/populations. Then verify stationary/walking/wind motion and full-quality2× frame/GPU costs; normal-footprint filtering or temporal AA remain separate measured options, not substitutes for silhouette quality.
