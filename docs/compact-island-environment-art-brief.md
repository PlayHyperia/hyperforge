...
Current status: **localhost:3333 still serves retained-world build104 and the v10 island assets as an ordinary playable preview.** The saved character/database and editable 2× resolution/High shadows remain intact. Density-only native217 was rejected; a distinct curved/twisted authored blade endpoint is now implemented in source, with actual material/root-storage bindings and 687 passing CPU/source regressions. Fixed-camera binary projected-coverage checks pass without adding plants. This is not native rendering or visual acceptance. Next is combined old/new swept-clearance certification at unchanged installed placements, then bounded replacement in the actual world and front/reverse visual review. No default promotion, measured performance gain or AAA acceptance.
...
- [ ] Add one explicit **combined-envelope certificate mode** to the existing grounding kernel, not a second independent fitter or global layout/worker-wire change. Sweep both coarse and authored vertices into per-blade/clump bounds BEFORE existing road/pad/water/support checks, sharing the existing work counter and continuation limit. Detached installed anchors, scales, normals, colors, tint and root deltas must compare bit-for-bit; retain installed visibility and reject the upgrade if any formerly visible blade fails. Fitter output is evidence only, never new installable placement. Fresh full terrain/input/constraint leases and publication-time revalidation remain mandatory; endpoint-only checks do not prove the transition clears an intervening hazard.
