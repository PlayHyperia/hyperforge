...
Current status: **localhost:3333 still serves retained-world build104 and the v10 island assets as an ordinary playable preview.** The saved character/database and editable 2× resolution/High shadows remain intact. Density-only native217 was rejected. The curved/twisted authored blade endpoint now has combined old/new clearance checks and bit-exact installed-placement acceptance; 927 CPU/source/worker regressions pass. This is not native rendering or visual acceptance. Next is the bounded actual-world replacement comparison from front/reverse views. No default promotion, measured performance gain or AAA acceptance.
...
- [ ] Finish/review the private frozen-camera split-draw adapter, then source-pin an isolated build and capture actual-world front/reverse OFF/ON comparisons. Original chunks must remain visible during preparation; accepted detail and coarse remainder swap atomically at zero weight and originals restore before disposal. All pending/active/retiring detail shares640 reservations. Static comparison is not continuous traversal, artist acceptance, GPU cost qualification or a completed meadow fix.

Evidence: service-layout `meadow-clearance-regression01`, `meadow-clearance-types01/02`, `meadow-clearance-lint01`, `meadow-clearance-worker-graph01`, `meadow-clearance-postflight01`; `meadow-authored-worker-safe01`; agent `meadow-authored-grounding-agent.json` and `-core01/02/03.log` (initial logs explicitly identify tool-output excerpts). The first two focused runs were45/48 and47/48; final48/48 is included in the927-test root rerun. No new image exists for this endpoint yet.
