...
Current status: **the newer 34-instance pond candidate passes source/build checks but has no new native views; AC power blocked capture before browser creation.** Historical matched captures below apply to the earlier 28-instance recipe only. Composition and performance remain unqualified; the grass-loading failure is not fixed.
