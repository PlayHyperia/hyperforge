...
Current status: **localhost:3333 still serves retained-world build104 and the v10 island assets as an ordinary playable preview.** The saved character/database and editable 2× resolution/High shadows remain intact. Private native24–26 completed 84 actual 2× images with zero GPU/shader errors and owned cleanup. The 4× local-density screen closes interior gaps and improves meadow volume from both camera directions, but harsh blade contrast and angular crossings remain. Next: retain that coverage target while qualifying coupled shading and bounded local infill; no island-wide density multiplier or candidate promotion. No measured performance gain or AAA acceptance.
