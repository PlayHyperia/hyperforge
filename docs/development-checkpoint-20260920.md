...
Current status: **localhost:3333 now serves retained-world build104 and the v10 island assets as an ordinary playable preview.** The saved character/database remain intact; the existing Brave session visibly renders the world with its 2× resolution and High shadows still selected. This fixes stale preview delivery, not AAA/performance acceptance. Native208's canopy-shading experiment remains rejected and its original source restored. Next target close blade silhouette and clump volume; sustained performance, motion/reverse/low-sun review, normal gameplay effects and loading reliability remain open. Historical outcomes remain recorded below.
