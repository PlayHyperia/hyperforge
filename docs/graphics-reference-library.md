...
Current status: **localhost:3333 still serves retained-world build104 and the v10 island assets as an ordinary playable preview.** The saved character/database and editable 2× resolution/High shadows remain intact. The bounded refinement prototype has actual-material/native 2× endpoint evidence, but its curve gain is modest and runtime integration/performance remain open. The turf-normal isolation now has six matched 2× image comparisons: it reduces secondary ground speckle, not the dominant ribbon/crossing noise. Native216 remains FAILED on final transient Docker availability; separate postflight and owned cleanup pass. Do not promote flat ground. Next: meaningful blade/field composition before more scalar tuning or refinement integration. No candidate promotion or AAA acceptance.
