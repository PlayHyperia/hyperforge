...
Current status: **source/build passed; native177 and instrumented native178 FAILED on landing-view grass preparation.** Three matched noon stills show a modest art improvement, not complete visual or performance acceptance. Completed native176 remains the full-route baseline.
...
- [ ] Before choosing the next grounding optimization, obtain exact served worker source/map provenance for reliable source attribution. Keep the 250 ms cap, density, geometry and quality unchanged; do not infer an exclusive hotspot from the last yield or a single long sampled interval. The two unequal pond-margin plant-massing proposals remain unimplemented, pending reliability work and full navigation/visual checks.
