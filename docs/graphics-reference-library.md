...
Current status: **scoped source checks pass; this remains an opt-in, visually unqualified candidate.** The grass-loading failure below is not fixed by this art change.
...
- [x] Record the broader mixed-fixture run honestly: current-v10 ecology is **nine passed / one failed / one skipped**. The historical fixed-arena-coordinate assertion fails before query/spawn and reproduces against exact pre-change HEAD code. The old mocked scenario is excluded. This is not a passing whole-suite claim.
...
- [ ] Compare separate, matched build89/build90 bank-detail captures before accepting the composition, root contact, crown overlap and rendering cost. The existing four-view route does not frame these pockets adequately; do not reorder it, warm it with extra views or waive its failures.
