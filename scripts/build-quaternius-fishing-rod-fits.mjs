...
async function main() {
  ...
  const manifest = JSON.parse(
    readFileSync(
      path.join(workspaceRoot, "scripts/quaternius-fishing-rod-fits.json"),
      "utf8",
    ),
  );
  ...
  if (options.check) {
    if (
      ...
      throw new Error("Quaternius fishing-rod fit report is stale");
    }
  } else {
    writeAtomic(reportPath, serialized);
  }
  process.stdout.write(
    `${options.check ? "Verified" : "Built"} ${report.outputs.length} isolated Quaternius fishing-rod fits\n`,
  );
}
